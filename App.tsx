        ...
        {status === ProcessingStatus.SUCCESS && result ? (
          ...
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center relative">
            ...
            <div className="relative z-20 animate-fade-in-up delay-200">
              ...
              {/* Mode Switcher Tabs - HUGE SIZE + GLASS + INERTIA */}
              <div className="flex gap-4 mb-8">
                  <button
                    onClick={() => setMode('auto')}
                    className={`
                      flex-1 py-12 rounded-3xl text-3xl font-bold uppercase tracking-wider transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] flex flex-col items-center justify-center gap-4 relative overflow-hidden group border
                      ${mode === 'auto' 
                        ? 'bg-indigo-600/40 backdrop-blur-2xl text-white shadow-[0_20px_50px_-12px_rgba(79,70,229,0.5)] border-indigo-500/50 scale-[1.02] z-10' 
                    ...
                    `}
                  >
                    <div className={`p-4 rounded-2xl transition-all duration-500 ${mode === 'auto' ? 'bg-indigo-500 text-white' : 'bg-white/5 text-gray-500 group-hover:text-white'}`}>
                  ...
                  <button
                    ...
                    className={`
                      flex-1 py-12 rounded-3xl text-3xl font-bold uppercase tracking-wider transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] flex flex-col items-center justify-center gap-4 relative overflow-hidden group border
                      ${mode === 'manual' 
                        ? 'bg-pink-600/40 backdrop-blur-2xl text-white shadow-[0_20px_50px_-12px_rgba(236,72,153,0.5)] border-pink-500/50 scale-[1.02] z-10' 
                        : 'bg-white/5 backdrop-blur-xl text-gray-500 border-white/5 hover:bg-white/10 hover:text-white hover:scale-[1.02] hover:border-white/20 hover:shadow-2xl'
                      }
                    `}
                  >
                    <div className={`p-4 rounded-2xl transition-all duration-500 ${mode === 'manual' ? 'bg-pink-500 text-white' : 'bg-white/5 text-gray-500 group-hover:text-white'}`}>
...
export default App;import React, { useState, useCallback, useEffect, useRef } from 'react';
import Header from './components/Header';
import UploadArea from './components/UploadArea';
import ResultCard from './components/ResultCard';
import ManualCropper from './components/ManualCropper';
